...
void main() {
	...
	vec2 offset = vec2(pixelOffset + 0.5) * samplePosMult * frameSizeRCP;
	
	/* Sample the 4 diagonal corners with equal weight */
	vec4 color = vec4(0.0);
	color += texture2D(texture, uv + vec2( offset.x,  offset.y)) * 0.25; // top-right
	color += texture2D(texture, uv + vec2(-offset.x,  offset.y)) * 0.25; // top-left  
	color += texture2D(texture, uv + vec2(-offset.x, -offset.y)) * 0.25; // bottom-left
	color += texture2D(texture, uv + vec2( offset.x, -offset.y)) * 0.25; // bottom-right
